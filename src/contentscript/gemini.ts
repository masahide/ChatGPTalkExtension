...
function injectText(text: string, autoSend: boolean) {
  ...
  if (autoSend) {
    // 送信ボタンの子要素に .send-button-icon がある
    const icon = document.querySelector(".send-button-icon");
    const btn = icon?.closest("button");
    console.log("[GeminiInjector] autoSend", { btn, icon });
    setTimeout(() => {
      btn?.click();
    }, 1000);
  }
...
function addMoreButton(
  ...
  btn.onclick = () => {
    ...
    btn.remove();
    if (rest.length)
      addMoreButton(rest.trim(), prompt, autoSend, max, title, url, partNo + 1);
  };
}
...
if (window !== window.top) {
  ...
  window.addEventListener("message", (e) => {
    ...
    const [first, rest] = splitTextAtNearestNewline(
      clean,
